...
$regnum = $_POST['txtregnum'];
$password = $_POST['txtpassword'];
$status = 1;


 $sql = "SELECT * FROM tblstudent WHERE regnum='" .$regnum . "' and password = '". $password."' and status = '". $status."'";
    $result = mysqli_query($conn,$sql);
    $row  = mysqli_fetch_array($result);
    
     $_SESSION["login_regnum"] = $row['regnum'];
  
     $count=mysqli_num_rows($result);
     if(isset($_SESSION["login_regnum"])) {
    {       
...
$error=' Wrong Reg No and Password';
         }
...
<body>
 ...
 <?php } ?></p>
                    <form action="" method="post">
                      <div class="mb-3">
                        <label for="userEmail" class="form-label">Reg No</label>
                        <input type="text" class="form-control" name="txtregnum" id="txtregnum" placeholder="Enter Reg No">
                      </div>
                      <div class="mb-3">
